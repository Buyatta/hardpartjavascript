...
//list of record id using map.
function printRecords(recordIds) {
  var records = recordIds.map(getstudentById);
  records.sort(function sortstudentAsc(record1, record2) {
...
function paidStudentsToEnroll() {
  var recordsToEnroll = studentRecords.filter(function needsToEnroll(record) {
...
function remindUnpaid(recordIds) {
  var unPaids = recordIds.filter(function notYetPaid(studentId) {
    var record = getstudentById(studentId);
    return !record.paid;
  });
  printRecords(unPaids);
}
